...
from schemas import SeatEvent, SeatEventType
from seat_state_machine import SeatStateMachine
from utils.detectors import detect_person_boxes, detect_loss_items
...
class CameraWorker :
    def __init__(self, camera_id, source, seat_rois, event_manager) :
        ...
        # Yolo 모델
        self.person_model = YOLO("../models/yolov11n.pt")
        self.lost_item_model = YOLO("../models/lost_item.pt")

        # 메인 루프 시작
        threading.Thread(target=self._loop, daemone=True).start()
    ...
    def start_lost_item_check(self, seat_id, usage_id) :
        ...
        self.lost_item_tartget_seat_id = seat_id
        self.usage_ids[seat_id] = usage_id
