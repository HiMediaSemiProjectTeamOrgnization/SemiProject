...
function KioskLogin({ onBack, onLoginSuccess }) {
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            // [핵심] 기간제 이용권이 있고, 할당된 고정석 ID가 있을 경우 선택 모달 표시
            if (data.has_period_pass && data.my_fixed_seat_id) {
                setLoginData(data);
                setShowFixSeatModal(true); 
            } else {
                // 고정석이 없으면 바로 좌석 선택 화면으로 이동
                onLoginSuccess(data); 
            }
    ...
    // [선택 2] 자유석 선택 (좌석 지도로 이동 -> 시간제 차감)
    const handleSelectFreeSeat = () => {
        setShowFixSeatModal(false);
        // 기존 로그인 성공 흐름(좌석 선택 화면)으로 진행
