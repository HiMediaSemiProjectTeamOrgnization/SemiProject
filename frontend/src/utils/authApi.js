...
export const authApi = {
    ...
    checkId: async (loginId) => {
        return await authClient.post('/auth/signup/check-id', { login_id: loginId });
    },
    // 값만 받아서 내부에서 객체로 포장 ({ phone: "..." })
    checkPhone: async (phone) => {
        return await authClient.post('/auth/signup/check-phone', { phone: phone });
    },
    // 값만 받아서 내부에서 객체로 포장 ({ input_code: "..." })
    checkVerifyPhone: async (inputCode) => {
        return await authClient.post('/auth/signup/check-verify-phone', { input_code: inputCode });
    },
    // 값만 받아서 내부에서 객체로 포장 ({ email: "..." })
    checkEmail: async (email) => {
        return await authClient.post('/auth/signup/check-email', { email: email });
    },
    // 값만 받아서 내부에서 객체로 포장 ({ input_code: "..." })
    checkVerifyEmail: async (inputCode) => {
        return await authClient.post('/auth/signup/check-verify-email', { input_code: inputCode });
    },
    // 추가 정보 기재 페이지
    ...
    onBoardCheckPhone: async (phone) => {
        return await authClient.post('/auth/google/onboarding/check-phone', { phone: phone });
    },
    // 값만 받아서 내부에서 객체로 포장 ({ input_code: "..." })
    onBoardCheckVerifyPhone: async (inputCode) => {
        return await authClient.post('/auth/google/onboarding/check-verify-phone', { input_code: inputCode });
    },
    // 아이디 찾기
